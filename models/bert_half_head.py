...
from models import revised_modules as my_module
...
class BertModel(nn.Module):
    def __init__(
        ...
        # # first encoder
        # first_layer = my_module.TransformerEncoderLayer(
        #     d_model, nhead, dim_feedforward=nhid, dropout=dropout,
        #     activation=activation, batch_first=True,
        #     self_layer_type="constraint_weight",
        # )
        # self.first_encoder = nn.TransformerEncoder(first_layer, 1)

        # bert encoder
        encoder_layers = my_module.TransformerEncoderLayer(
            d_model, nhead, dim_feedforward=nhid, dropout=dropout,
            activation=activation, batch_first=True
        )
    ...
    def forward(self, Xs, age, pos):
        ...
        # # first encoder
        # output = self.first_encoder(output, src_key_padding_mask=src_key_padding_mask)

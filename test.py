...
from models.metric import MetricTracker, NA_curve, KM_curve
...
def test(config):
    ...
    # datasets
    test_datasets = dict()
    # share same config with train except mode = "test"
    keys = ["datasets", "train"]
    name = "data"
    ...
    for k in range(k_fold):
        ...
        event_times, targets, outputs, test_log = tester.test()
        ...
        # # plot NA curve
        # NA_curve(event_times, targets, outputs, threshold)
        # # plot KM curve
        # KM_curve(event_times, targets, outputs, threshold)
    ...
    if config.test_args.bootstrapping:
        ...
        bootstrapping(event_times, targets, outputs, metrics_epoch, test_metrics, repeat=N)
